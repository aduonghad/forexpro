import { Candle, Timeframe, TradingSymbol } from '../types';
...
export function getPipSize(symbol: TradingSymbol): number {
  switch (symbol) {
...
export function analyzeClosedCandle(
  candles: Candle[],
  ...
  // Lấy cây nến vừa đóng hoàn chỉnh:
  // Nếu có từ 2 nến trở lên: nến candles[candles.length - 2] là nến vừa đóng, candles[candles.length - 1] là nến đang chạy.
  ...
  // 1. KIỂM TRA MÔ HÌNH 3 NẾN (TRIPLE PATTERNS)
  if (prev && prev2) {
    ...
    // Morning Star (Sao Mai Đảo Chiều Tăng)
    ...
    // Evening Star (Sao Hôm Đảo Chiều Giảm)
    ...
    // Three White Soldiers (Ba Chàng Lính Ngự Lâm)
      ...
      patternType = 'BULLISH';
      patternCategory = 'TRIPLE';
      sentiment = 'Ba nến xanh tăng liên tiếp với thân nến mạnh mẽ, thể hiện xung lực mua áp đảo không ngừng nghỉ.';
      confidence = 'HIGH';
      priceActionSignal = 'CANH_MUA';
    }
  ...
  // 2. KIỂM TRA MÔ HÌNH 2 NẾN (DOUBLE PATTERNS) - nếu chưa khớp 3 nến
  if (patternCategory === 'SINGLE' && prev) {
    ...
    // Piercing Line (Đường Xuyên Tăng)
    ...
    // Bullish Harami (Mẹ Bồng Con Tăng)
    ...
    // Bearish Harami (Mẹ Bồng Con Giảm)
  ...
  // 3. KIỂM TRA MÔ HÌNH NẾN ĐƠN (SINGLE CANDLE PATTERNS) - nếu chưa khớp mô hình đa nến
  if (patternCategory === 'SINGLE' && patternName.includes('Tiêu Chuẩn')) {
    ...
    // 3.3 Shooting Star (Sao Băng Giảm)
    ...
    // 3.6 Marubozu
      ...
      if (isBullish) {
        ...
        patternType = 'BULLISH';
        sentiment = `Thân nến xanh siêu dài chiếm ${bodyPercent}% biên độ (${bodyPips} pips), phe Mua áp đảo toàn diện từ mở đến đóng nến.`;
        confidence = 'HIGH';
        priceActionSignal = 'CANH_MUA';
      } else {
  ...
  return {
    ...
    comparison: prev ? {
      isEngulfing: body > Math.abs(prev.close - prev.open),
      higherHigh: high > prev.high,
      lowerLow: low < prev.low
    } : undefined
  };
}
